package com.sampleroom.mobile.update
...
class AppUpdateClient(
    private val clientType: String = "phone",
    private val http: OkHttpClient = OkHttpClient.Builder()
        .connectTimeout(4, TimeUnit.SECONDS)
        .readTimeout(8, TimeUnit.SECONDS)
        .build()
) {
    suspend fun latest(baseUrl: String): AppReleaseInfo? = withContext(Dispatchers.IO) {
        ...
        http.newCall(request).execute().use { response ->
            ...
            val release = JSONObject(body).optJSONObject("release") ?: return@withContext null
