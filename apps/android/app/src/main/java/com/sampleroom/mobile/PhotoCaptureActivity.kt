...
/** Pad 同源的多图相机会话：连续拍摄、现场删图，完成后一次性返回。 */
class PhotoCaptureActivity : ComponentActivity() {
    ...
    private val maxPhotos by lazy {
        intent.getIntExtra(EXTRA_MAX_PHOTOS, MAX_PHOTOS).coerceIn(1, MAX_PHOTOS)
    }

    private val cameraPermissionLauncher = registerForActivityResult(
        ActivityResultContracts.RequestPermission()
    ) { granted ->
        if (granted) bindCamera() else showCameraError("未获得相机权限，无法拍照")
    }
    ...
    private fun buildUi() {
        ...
        topBar.addView(
            iconButton(R.drawable.ic_camera_back, "返回") { cancelAndFinish() },
        ...
        thumbnailScroll = HorizontalScrollView(this).apply { isHorizontalScrollBarEnabled = false }
        ...
        finishButton = actionText("完成（0）", primary = true) { submitMultiple() }.apply {
            isEnabled = false
            alpha = 0.45f
        }
    ...
    private fun capturePhoto() {
        ...
        if (captureInFlight || capturedFiles.size >= maxPhotos) return
        captureInFlight = true
        shutter.isEnabled = false
        shutter.performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP)
        val target = File.createTempFile("phone-qc-photo-", ".jpg", File(cacheDir, "qc-camera").apply { mkdirs() })
        capture.takePicture(
    ...
    private fun updateMultipleControls() {
        ...
        countLabel.text = if (count >= maxPhotos) "已达 $maxPhotos 张上限" else "已拍 ${count} 张"
        finishButton.text = "完成（${count}）"
        finishButton.isEnabled = count > 0
        finishButton.alpha = if (count > 0) 1f else 0.45f
        shutter.isEnabled = count < maxPhotos
        shutter.alpha = if (shutter.isEnabled) 1f else 0.45f
    ...
    private fun submitMultiple() {
        ...
        val files = capturedFiles.toList()
        val uris = files.map(::fileUri)
        val result = Intent()
            .putStringArrayListExtra(EXTRA_URIS, ArrayList(uris.map { it.toString() }))
            .putStringArrayListExtra(EXTRA_PATHS, ArrayList(files.map(File::getAbsolutePath)))
    ...
    companion object {
        ...
        const val EXTRA_PATHS = "photo_paths"
        const val EXTRA_MAX_PHOTOS = "max_photos"
