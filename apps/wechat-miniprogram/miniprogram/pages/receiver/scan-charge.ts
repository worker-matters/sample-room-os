...
import {
  createReceiverScanCharge,
  resolveReceiverScanCharge,
  uploadReceiverChargeAttachment,
  voidReceiverCharge
} from "../../services/apiClient";
import { isActiveReceiverIdentity, requireReceiverApiContext } from "../../services/receiverSession";
import type { ReceiverCharge, ReceiverScanChargeContext } from "../../types/contracts";
...
type DisplayContext = Omit<ReceiverScanChargeContext, "charges"> & { charges: DisplayCharge[] };
    ...
    : role === "planner" ? "计划员"
      : role === "boss" ? "老板"
        : role === "system_owner" ? "System Owner"
          : role;
const presentContext = (context: ReceiverScanChargeContext): DisplayContext => ({
  ...context,
...
Page({
  ...
  async scanOrderQr() {
    try {
      ...
      const parsed = parseMiniappOrderQrPayload(scanned.result);
      await this.load(parsed.token);
    } catch (error) {
  ...
  async load(providedToken?: string) {
    ...
    try {
      const app = getApp<{ globalData: MiniappGlobalData }>();
      const api = await requireReceiverApiContext(app.globalData);
      const context = await resolveReceiverScanCharge(api.baseUrl, api.sessionToken, token);
  ...
  resetScan() {
    this.setData({ token: "", context: null, charge: { name: "", amount: "", explanation: "" }, chargeFiles: [], message: "" });
  },
  ...
  async submitCharge() {
    ...
    try {
      const app = getApp<{ globalData: MiniappGlobalData }>();
      const api = await requireReceiverApiContext(app.globalData);
      const created = await createReceiverScanCharge(api.baseUrl, api.sessionToken, this.data.token, {
        name: this.data.charge.name.trim(),
        amount,
        explanation: this.data.charge.explanation.trim(),
        sourceScene: "receiver_mobile_scan"
      });
      let failed = 0;
      for (const file of this.data.chargeFiles) {
        try {
          await uploadReceiverChargeAttachment(api.baseUrl, api.sessionToken, created.orderId, created.charge.id, file.path);
        } catch {
          failed += 1;
        }
      }
  ...
  async voidCharge(event: WechatMiniprogram.TouchEvent) {
    ...
    try {
      const app = getApp<{ globalData: MiniappGlobalData }>();
      const api = await requireReceiverApiContext(app.globalData);
      await voidReceiverCharge(api.baseUrl, api.sessionToken, orderId, chargeId);
      await this.load(this.data.token);
    } catch (error) {
